...
def to_archive_xl(filename):
    with open(filename, "w") as file:
        ...
        for sectorPath in deletions:
            ...
            if sectorData is None or len(sectorData) == 0:
                continue
            ...
            # keep track of how many nodes we have removed so that we don't swallow the last node
            counter = 1
            for empty_collection in sectorData:
                if empty_collection["nodeDataIndex"] > currentNodeIndex or counter == len(sectorData):
